...
export function Rail() {
    ...
    useEffect(() => {
        const onScroll = () => {
            ...
            let current = 'violet';
            for (const el of document.querySelectorAll('[data-band]')) {
                const box = el.getBoundingClientRect();
                if (box.top <= middle && box.bottom >= middle)
                    current = el.dataset.band ?? current;
            }
            setBand(current);
        };
        onScroll();
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "rail", "aria-hidden": true }), _jsx("div", { className: "rail-thumb", "data-band": band, style: { top }, "aria-hidden": true })] }));
}
